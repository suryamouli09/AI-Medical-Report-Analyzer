...
import re
import io
from analyzer import find_best_parameter_match, extract_value_from_line, is_header_footer_line
...
def extract_parameters_from_tables(tables):
    ...
    for df in tables:
        for row in df.values:
            row_cells = [str(x).strip() for x in row if x and str(x).strip()]
            if not row_cells:
                continue

            row_text = " ".join(row_cells)

            if is_header_footer_line(row_text):
                continue
            ...
            val = extract_value_from_line(row_text, matched_param)
